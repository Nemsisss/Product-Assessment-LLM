...
- Explore AWS containers that come with GPUs (such as g5g.2xlarge) and replace OpenAI with llama2
    - The program code does not need to be changed, the lines of code that I wrote for using llama2 as LLM are still in the program, they just need to be uncommented and the code for calling OpenAI needs to be removed. The steps to follow for switching the model to llama2 are documented in the app’s Github repository’s ReadMe at https://github.com/Nemsisss/Product-Assessment-LLM#switching-to-llama2.
